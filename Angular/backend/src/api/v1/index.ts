import { selfInformationController } from "./self-information/self-information.controller";
...
apiV1Router
  ...
  .use("/socket-events", new SocketEventsController().applyRoutes())

  // Sockets events routes
  .use("/files", new FilesController().applyRoutes())

  // Task routes
  .use("/tasks", new ResourceController<ITask>(TaskModel).applyRoutes())

  // Example routes
  .use("/example", new ExampleController().applyRoutes())

  .use("/contacts", new ContactsController().applyRoutes())

  .use("/messages", new MessagesController().applyRoutes())

  .use("/tv", new TVController().applyRoutes())

  .use("/user", new UserController().applyRoutes())

  .use("/pinboard", new PinboardController().applyRoutes())

  /* Cretanian */
  .use("/smarthome-list", new SmarthomeListController().applyRoutes())

  .use("/online-devices-list", new OnlineDevicesListController().applyRoutes())

  .use("/files-headers", new FilesHeadersController().applyRoutes())

  .use("/chat-headers", new ChatHeadersController().applyRoutes())
  ...
  .use("/in-out-devices", new InOutDevicesController().applyRoutes())

  .use("/out-video", new OutVideoController().applyRoutes())

  .use("/teams-list", new TeamsListController().applyRoutes())

  .use("/participants", new ParticipantsController().applyRoutes())

  .use("/activity", new ActivityController().applyRoutes())

  .use("/tvgrid", new TvGridController().applyRoutes())
  .use("/selfInformation", new selfInformationController().applyRoutes());
